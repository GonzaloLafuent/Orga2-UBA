...
paddr_t mmu_init_task_dir(paddr_t phy_start) {
  ...
  mmu_map_page(cr, TASK_SHARED_PAGE, SHARED, (MMU_P | MMU_W| MMU_U)); // Supongo que Lectura y escritura, supervirsor y presente
...
bool page_fault_handler(vaddr_t virt) {
  ...
  if(ON_DEMAND_MEM_START_VIRTUAL >= virt && ON_DEMAND_MEM_END_VIRTUAL<=virt){
    ...
    if((pageDirectoryEntry.attrs & MMU_P) == 0 || (pageTable->attrs || MMU_P) == 0){
